...
@ViewScoped
@Data
public class PropietarioAgregarFaces implements Serializable, JmoordbCoreXHTMLUtil, IPaginator, SprintFacesServices, FacesWebSocket, FacesServices {
...
// </editor-fold>
    // <editor-fold defaultstate="collapsed" desc="String agregarmeComoPropietario(">

    public String agregarmeComoPropietario() {
        try {
            ...
            ActionHistory actionHistory = new ActionHistory.Builder()
                    .iduser(userLogged.getIduser())
                    .fecha(JmoordbCoreDateUtil.fechaHoraActual())
                    .evento("agregarme como propietario")
                    .clase(FacesUtil.nameOfClass())
                    .metodo(FacesUtil.nameOfMethod())
                    .build();
         ...
         Integer c=0;
         for(ProyectoMiembro pm:proyectoReabrir.getProyectoMiembro()){
             if(pm.getUserView().getIduser().equals(userLogged.getIduser())){
                 found = Boolean.TRUE;
                 if(!pm.getPropietario()){
                     proyectoReabrir.getProyectoMiembro().get(c).setPropietario(Boolean.TRUE);   
         ...
         if(!found){
             UserView uv  = new UserView(userLogged.getIduser(), userLogged.getName(), userLogged.getPhoto(), userLogged.getEmail(),userLogged.getRecibirNotificacion());
                ProyectoMiembro proyectoMiembro = new ProyectoMiembro(Boolean.TRUE, uv, Boolean.TRUE);
               proyectoReabrir.getProyectoMiembro().add(proyectoMiembro);
            ...
            if (!proyectoServices.update(proyectoReabrir)) {
                ...
                MessageWebSocket messageWebSocket = new MessageWebSocket.Builder()
                        ...
                        .message("agregarme como propietario")
                        .iduser(userLogged.getIduser())
...
// </editor-fold>
    ...
    public String cerrarDialogo() {
        try {
            ...
            closeOverlayPanel("PF('overlayPanelPropietarioAgregar').hide()");
        } catch (Exception e) {
            FacesUtil.errorMessage(FacesUtil.nameOfClassAndMethod() + " " + e.getLocalizedMessage());
        }
