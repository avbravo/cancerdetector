...
@ViewScoped
@Data
public class PapeleraReciclajeProyectoFaces implements Serializable, JmoordbCoreXHTMLUtil, IPaginator, SprintFacesServices, FacesWebSocket, FacesServices {
// <editor-fold defaultstate="collapsed" desc="ConverterServices">
    ...
    private Proyecto proyectoPapeleraReciclaje = new Proyecto();
        ...
        try {
            ...
            document.add(
                    ReportUtils.paragraph(
                            rf.fromMessage("print.titlesproyectoseliminados"), FontFactory.getFont("arial", 12, Font.BOLD), Element.ALIGN_CENTER));
...
// </editor-fold>
    // <editor-fold defaultstate="collapsed" desc="String prepareProyectoPapeleraReciclaje(Proyecto proyecto)">

    public String prepareProyectoPapeleraReciclaje(Proyecto proyecto) {
        try {
            dialogVisibleWebSocket= Boolean.TRUE;
            JmoordbCoreUtil.copyBeans(proyectoPapeleraReciclaje, proyecto);
            proyectoMiembroOldList = proyectoPapeleraReciclaje.getProyectoMiembro();
...
// </editor-fold>
    // <editor-fold defaultstate="collapsed" desc="String sacarProyectoDePapelera()">

    public String sacarProyectoDePapelera() {
        try {

            Proyecto proyectoDBNow = proyectoServices.findByIdproyecto(proyectoPapeleraReciclaje.getIdproyecto()).get();
             ...
             * pendientes
             */
            proyectoPapeleraReciclaje.setEstado(rf.fromMessage("selectonemenu.iniciadovalue"));
            ...
            proyectoPapeleraReciclaje.getActionHistory().add(actionHistory);

            if (!proyectoServices.update(proyectoPapeleraReciclaje)) {
                ...
                sendEmailEditProject(proyectoPapeleraReciclaje);
                /**
                 * WebSocket
                 */
                userViewForWebSocketList = userViewFromProjectExclude(proyectoPapeleraReciclaje, Boolean.FALSE, userLogged);
                MessageWebSocket messageWebSocket = new MessageWebSocket.Builder()
                        ...
                        .key("proyecto")
                        .value(proyectoPapeleraReciclaje.getIdproyecto() != null ? proyectoPapeleraReciclaje.getIdproyecto().toString() : "")
                        .message("PapeleraReciclaje proyecto")
                        .iduser(userLogged.getIduser())
                        .date(JmoordbCoreDateUtil.fechaHoraActual())
                        .build();
                sendMensajesWebsocket(messageWebSocket, Boolean.TRUE);

                closeOverlayPanel("PF('overlayPanelProyectoPapeleraReciclaje').hide()");
                refresh();
            }
        } catch (Exception e) {
            FacesUtil.errorMessage(FacesUtil.nameOfClassAndMethod() + " " + e.getLocalizedMessage());
