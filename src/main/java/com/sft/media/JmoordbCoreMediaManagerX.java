...
@Named
@RequestScoped
public class JmoordbCoreMediaManagerX implements Serializable {
// <editor-fold defaultstate="collapsed" desc="field">
    ...
    public void init() {
        try {
            ...
            String fileDownloadPath = (String) JmoordbCoreMediaContext.get("pathOfFile2");
            String nameFileDownloadPath = (String) JmoordbCoreMediaContext.get("nameOfFile2");
            
            ConsoleUtil.test("\t\t?????????????????????????????????????????????????????????????????????");
            ConsoleUtil.test("\t\t"+FacesUtil.nameOfClassAndMethod() + " " );
            ConsoleUtil.test("\t\tfileDownloadPath"+fileDownloadPath+ " " );
            ConsoleUtil.test("\t\tnameFileDownloadPath"+nameFileDownloadPath+ " " );
            ConsoleUtil.test("\t\t?????????????????????????????????????????????????????????????????????");
